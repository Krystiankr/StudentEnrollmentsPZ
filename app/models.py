...
class Transfer(models.Model):
    ...
    requester_student_ID = models.ForeignKey(
        Studenci, on_delete=models.CASCADE, related_name="requested_transfers"
    )
    exchanger_student_ID = models.ForeignKey(
        Studenci, on_delete=models.CASCADE, related_name="exchanged_transfers"
    )
...
class CzatPrywatny(models.Model):
    ...
    tresc_wiadomosci = models.CharField(max_length=255, null=True)
    data_wiadomosci = models.DateField(null=True)
    ID_studenta_nadawcy = models.ForeignKey(
        Studenci, on_delete=models.CASCADE, related_name="nadawca"
    )
    ID_studenta_odbiorcy = models.ForeignKey(
        Studenci, on_delete=models.CASCADE, related_name="odbiorca"
    )
